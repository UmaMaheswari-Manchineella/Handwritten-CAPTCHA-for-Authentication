...
@app.route('/')
def index():
  index.key1=random.choice(symbols)
  index.key2=random.choice(symbols)
...
#loading the model
  model = tf.keras.models.load_model('D:/flask_app/symbolsnew.h5',custom_objects={'softmax_v2': tf.nn.softmax})
...
#cropping the canvas image
  ...
  for i in range (0,2):
    ...
    #predicting the cropped images
      ...
      if i==1:
          out2=list1[y]
          if index.key1==out1 and index.key2==out2:
